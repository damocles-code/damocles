RECURSIVE SUBROUTINE propagate(nu_p,dir_cart,pos_cart,iG_axis,lgabs,lgactive,w,scatno)
    ...
    DO ispec=1,dust%n_species
        ...
        wavid=MINLOC(ABS((dust%species(ispec)%wav(:)-(c*1e6/nu_p))))
        wavid2=wavid(1)

        !calculate opactiy as function of rho for specific wavelength by interpolating between bins for each species
        IF ((c*1e6/nu_p-dust%species(ispec)%wav(wavid2))<0) THEN
            kappa_i=dust%species(ispec)%ext_opacity(wavid2)-((dust%species(ispec)%ext_opacity(wavid2)-dust%species(ispec)%ext_opacity(wavid2-1))* &
                & ((dust%species(ispec)%wav(wavid2)-c*1e6/nu_p)/(dust%species(ispec)%wav(wavid2)-dust%species(ispec)%wav(wavid2-1))))
        ELSE
            kappa_i=dust%species(ispec)%ext_opacity(wavid2)+((dust%species(ispec)%ext_opacity(wavid2+1)-dust%species(ispec)%ext_opacity(wavid2))* &
                & ((c*1e6/nu_p-dust%species(ispec)%wav(wavid2))/(dust%species(ispec)%wav(wavid2+1)-dust%species(ispec)%wav(wavid2))))
        END IF
        ...
        !NOTE THAT ONE DOES NOT JUST WEIGHT THE ALBEDO 
                IF ((c*1e6/nu_p-dust%species(ispec)%wav(wavid2))<0) THEN
            kappa_sca_i=dust%species(ispec)%sca_opacity(wavid2)-((dust%species(ispec)%sca_opacity(wavid2)-dust%species(ispec)%sca_opacity(wavid2-1))* &
                & ((dust%species(ispec)%wav(wavid2)-c*1e6/nu_p)/(dust%species(ispec)%wav(wavid2)-dust%species(ispec)%wav(wavid2-1))))
        ELSE
            kappa_sca_i=dust%species(ispec)%sca_opacity(wavid2)+((dust%species(ispec)%sca_opacity(wavid2+1)-dust%species(ispec)%sca_opacity(wavid2))* &
                & ((c*1e6/nu_p-dust%species(ispec)%wav(wavid2))/(dust%species(ispec)%wav(wavid2+1)-dust%species(ispec)%wav(wavid2))))
        END IF
...
!v=dust_geometry%v_max*((r/(dust_geometry%R_max*1e5))**l)
!vel_vect=normalise(pos_cart)*v
