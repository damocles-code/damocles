        ...
        SELECT CASE(gas_geometry%type)
            ...
            CASE("arbitrary")
                ...
                ELSE
                    ...
                    !number of packets to be emitted in each cell scaled with number density of particles in cell
                    NP(:)=NINT((grid_cell(:)%nrho**2)*real(n_packets)*(grid_cell(ii)%vol/SUM((grid_cell%nrho**2)*(grid_cell%vol))))
                    gas_geometry%v_max=dust_geometry%v_max
                    gas_geometry%R_max=dust_geometry%R_max
                    gas_geometry%v_power=dust_geometry%v_power
