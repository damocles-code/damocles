                ...
                if (dust_geometry%lg_clumped) then
                   ...
                   do ii=1,mothergrid%tot_cells
                      ...
                      if ((grid_cell(ig)%r<dust_geometry%r_max_cm) .and. (grid_cell(ig)%r>dust_geometry%r_min_cm)) then
                         ...
                         !when the clump no. density distrib of the dust is set to 3, then it produces an Infinity term in the 4th term in the eqn. for n_clumps_per_cell and clumps end up not being created in those cells. changing the number to a non-integer no. v close to 3 fixes this. 
                         if (dust_geometry%clump_power == 3) then
                              dust_geometry%clump_power = 3.000001   
                         end if
