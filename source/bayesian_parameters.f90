...
contains
  ...
  subroutine read_bayesian_params(params,flags)
    ...
    dust_geometry%v_min = dust_geometry%v_max*dust_geometry%r_ratio
 
 !in case that you've set velocity to be coupled to radius and are not varying rmax in the sampler,
           !calculate the values of rmax in params(12,:) using the values of vmax
    if ((flags(12,i_line) == 0) .or. (gas_geometry%r_max == 0.0)) then
           rmax_vals(:) = params(11,:)*1000*day_no*8.64e-6
           gas_geometry%r_max=rmax_vals(i_line)
           gas_geometry%r_min=gas_geometry%r_max*gas_geometry%r_ratio
    else
           rmax_vals(:) = params(12,:)
    end if
  
    ! find max gas rmax to set bounds of sn in build_dust_grid
  
    max_gas_rmax = maxval(rmax_vals)
