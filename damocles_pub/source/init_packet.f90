...
contains
  ...
  SUBROUTINE emit_photon(nu_p,dir_cart,pos_cart,iG_axis,lgactive,w,cell_iG)
    ...
    IF (clumpgas==1) THEN
       POS_CART(:)=(/ (mgrid(cell_iG)%axis(1)+RANDOM(1)*width(1)),(mgrid(cell_iG)%axis(2)+RANDOM(2)*width(2)),(mgrid(cell_iG)%axis(3)+RANDOM(3)*width(3))/)
    ...
    IF (((POS_SPH(1) > R_min_gas) .AND. (POS_SPH(1) < R_max_gas) .AND. (clumpgas==0)) .OR. (clumpgas==1)) THEN                       !If the photon lies inside the radial bounds of the supernova then it is processed       
