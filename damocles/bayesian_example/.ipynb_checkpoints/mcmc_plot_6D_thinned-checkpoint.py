...
r"$v_{\rm min}$"
                                ...
                                r"$(10^3{\rm km s}^{-1})$",
                                r"$\beta_v$",
				r"$\log \ (M_{\rm d}/M_{\odot})$",r"$\log \ (a/{\rm \mu m})$",#r"$F_{\rm 6300}/F_{\rm 6363}$"#,"$f$",r"$\beta_{\rm \, clump}$"
]
nwalkers = 500
...
data = np.genfromtxt('chain.dat',usecols=(1,2,3,4,5,6))
...
for i in range(ndim):
	print i*6
	fig1.axes[i*6].set_ylabel(titles_units[i],fontsize = 20)

	if i>0:
		fig1.axes[i*6].tick_params(axis='y',labelleft = True)
	#	fig1.axes[i*6].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
	fig1.axes[30+i].set_xlabel(titles_units[i],fontsize = 20)
	fig1.axes[30+i].tick_params(axis='x',labelbottom = True)
	fig1.axes[30+i].xaxis.set_major_formatter(FormatStrFormatter('%.1f'))
...
for i in range(ndim):
        print i*6
        fig1.axes[i*6].set_ylabel(titles_units[i])
        for j in range(i,ndim-1):
                fig1.axes[i+(j+1)*6].tick_params(labelsize = 16)
                fig1.axes[i+(j+1)*6].plot(best_fit_vals[i],best_fit_vals[j+1],marker='o',markerfacecolor='magenta',markeredgecolor='none',linestyle='none')
                fig1.axes[i+(j+1)*6].set_xlabel(titles_units[i])
...
plt.savefig('05ip_d4075_Halpha.pdf')

#plot the autocorrelation for each parameter
#for i in range(ndim):
#    plt.figure()
#    g = emcee.autocorr.function(np.mean(chain[:, :, i], axis=1))
#    plt.plot(g)
#    plt.title(titles[i])
